...
func (r *ftResult) CompareToExp(got *ftExecution, exp *ftExpected) {
	if exp.Status != got.status {
		r.statusFail = NOK
	}
	r.stdoutFail = compareOutput(exp.getExpOut(), got.outBuf.String())
	r.stderrFail = compareOutput(exp.getExpErr(), got.errBuf.String())
}
